...
/**
 * Resource class to handle the StateHistory Entity.
 *
 * @author Robin
 */
@Path("/statehistory")
...
@KCBSecure
public class StateHistoryResource implements CRUDResource<StateHistory> {

    /**
     * Logger object of the StateHistoryResource class
     */
    private static final Logger logger = LogManager.getLogger(StateHistoryResource.class);

    /**
     * StateHistory Service to handle stateHistory related CRUD operations
     */
    @Inject
    ...
    /**
     * Create a new StateHistory from request body.
     *
    ...
    public String create(StateHistory entity) {
        try {
    ...
    public String delete(String requestBody) {
        ...
        try {
            StateHistory stateHistory = stateHistoryService.getById(parsedBody.get("id").getAsString());
            stateHistoryService.delete(stateHistory);
            return makeResponseMessage("StateHistory deleted successfully.");
        } catch (InvalidIdException | DeletionException e) {
            logger.debug("Error occurred during stateHistory deletion {}", e.getMessage());
            return makeResponseMessage("StateHistory deletion failed.");
        }
    }
    ...
    public String getAll() {
        List<StateHistory> stateHistories = stateHistoryService.getAll();
    ...
    public String getById(@PathParam("id") String id) {
        try {
            StateHistory stateHistory = stateHistoryService.getById(id);
            return stateHistory != null ? json.marshal(stateHistory) : makeResponseMessage("StateHistory not found.");
        } catch (InvalidIdException e) {
            logger.debug("Error occurred during stateHistory fetching {}", e.getMessage());
            return makeResponseMessage("Invalid id. StateHistory couldn't be fetched.");
        }
    }
