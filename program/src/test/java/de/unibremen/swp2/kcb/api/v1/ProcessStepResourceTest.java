...
/**
 * The type Process step resource test.
 *
 * @author Marius
 */
public class ProcessStepResourceTest {
     ...
     */
    @InjectMocks
    private ProcessStepResource resource;
     ...
     */
    @Mock
    private ProcessStep mockedProcessStep;
    ...
    public void testCreateNormal() throws CreationException, EntityAlreadyExistingException {
        when(service.create(any())).thenReturn(mockedProcessStep);
        when(jsonUtil.marshal(any())).thenReturn("");
        resource.create(mockedProcessStep);
        verify(service, times(1)).create(mockedProcessStep);
    }
    ...
    public void testUpdateNormal() throws UpdateException, InvalidIdException {
        when(service.update(mockedProcessStep)).thenReturn(mockedProcessStep);
        when(service.getById(any())).thenReturn(mockedProcessStep);
        when(jsonUtil.marshal(any())).thenReturn("");
        resource.update(mockedProcessStep);
        verify(service, times(1)).update(mockedProcessStep);
    }
    ...
    public void testUpdateFail() throws UpdateException, InvalidIdException {
        when(service.update(mockedProcessStep)).thenThrow(UpdateException.class);
        when(service.getById(any())).thenReturn(mockedProcessStep);
        when(jsonUtil.marshal(any())).thenReturn("");
        String response = resource.update(mockedProcessStep);
        verify(service, times(1)).update(mockedProcessStep);
    }
