...
/**
 * The type Parameter resource test.
 *
 * @author Marius
 */
public class ParameterResourceTest {
     ...
     */
    @Mock
    private Parameter mockedParameter;
    ...
    public void testCreateNormal() throws CreationException, EntityAlreadyExistingException {
        when(service.create(any())).thenReturn(mockedParameter);
        when(jsonUtil.marshal(any())).thenReturn("");
        resource.create(mockedParameter);
        verify(service, times(1)).create(mockedParameter);
    }
    ...
    public void testUpdateNormal() throws UpdateException, InvalidIdException {
        when(service.update(mockedParameter)).thenReturn(mockedParameter);
        when(service.getById(any())).thenReturn(mockedParameter);
        when(jsonUtil.marshal(any())).thenReturn("");
        resource.update(mockedParameter);
        verify(service, times(1)).update(mockedParameter);
    }
    ...
    public void testUpdateFail() throws UpdateException, InvalidIdException {
        when(service.update(mockedParameter)).thenThrow(UpdateException.class);
        when(service.getById(any())).thenReturn(mockedParameter);
        when(jsonUtil.marshal(any())).thenReturn("");
        String response = resource.update(mockedParameter);
        verify(service, times(1)).update(mockedParameter);
    }
