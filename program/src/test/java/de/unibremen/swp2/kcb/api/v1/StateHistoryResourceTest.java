...
/**
 * The type State history resource test.
 *
 * @author Marius
 */
public class StateHistoryResourceTest {

    /**
     * Injected instance of resource
     */
    @InjectMocks
    private StateHistoryResource resource;
     ...
     */
    @Mock
    private StateHistory mockedStateHistory;
    ...
    public void testCreateNormal() throws CreationException {
        when(service.create(any())).thenReturn(mockedStateHistory);
        when(jsonUtil.marshal(any())).thenReturn("");
        resource.create(mockedStateHistory);
        verify(service, times(1)).create(mockedStateHistory);
    }
    ...
    public void testUpdateNormal() throws UpdateException, InvalidIdException {
        when(service.update(mockedStateHistory)).thenReturn(mockedStateHistory);
        when(service.getById(any())).thenReturn(mockedStateHistory);
        when(jsonUtil.marshal(any())).thenReturn("");
        resource.update(mockedStateHistory);
        verify(service, times(1)).update(mockedStateHistory);
    }
    ...
    public void testUpdateFail() throws UpdateException, InvalidIdException {
        when(service.update(mockedStateHistory)).thenThrow(UpdateException.class);
        when(service.getById(any())).thenReturn(mockedStateHistory);
        when(jsonUtil.marshal(any())).thenReturn("");
        String response = resource.update(mockedStateHistory);
        verify(service, times(1)).update(mockedStateHistory);
    }
