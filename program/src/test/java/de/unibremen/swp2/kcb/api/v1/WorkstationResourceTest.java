...
/**
 * The type Workstation resource test.
 *
 * @author Marius
 */
public class WorkstationResourceTest {
     ...
     */
    @Mock
    private Workstation mockedWorkstation;
    ...
    public void testCreateNormal() throws CreationException, EntityAlreadyExistingException {
        when(service.create(any())).thenReturn(mockedWorkstation);
        when(jsonUtil.marshal(any())).thenReturn("");
        resource.create(mockedWorkstation);
        verify(service, times(1)).create(mockedWorkstation);
    }
    ...
    public void testUpdateNormal() throws UpdateException, InvalidIdException {
        when(service.update(mockedWorkstation)).thenReturn(mockedWorkstation);
        when(service.getById(any())).thenReturn(mockedWorkstation);
        when(jsonUtil.marshal(any())).thenReturn("");
        resource.update(mockedWorkstation);
        verify(service, times(1)).update(mockedWorkstation);
    }
    ...
    public void testUpdateFail() throws UpdateException, InvalidIdException {
        when(service.update(mockedWorkstation)).thenThrow(UpdateException.class);
        when(service.getById(any())).thenReturn(mockedWorkstation);
        when(jsonUtil.marshal(any())).thenReturn("");
        String response = resource.update(mockedWorkstation);
        verify(service, times(1)).update(mockedWorkstation);
    }
