...
/**
 * The type State exec resource test.
 *
 * @author Marius
 */
public class StateExecResourceTest {

    /**
     * Injected instance of resource
     */
    @InjectMocks
    private StateExecResource resource;
     ...
     */
    @Mock
    private StateExecService service;

    /**
     * Mocked version of model
     */
    @Mock
    private StateExec mockedStateExec;
    ...
    public void testCreateNormal() throws CreationException {
        String response = resource.create(mockedStateExec);
        assertTrue("StateExecs can't be created.", response.contains("can not be created"));
    }
    ...
    public void testCreateFail() throws CreationException {
        ...
        String response = resource.create(mockedStateExec);
        assertTrue("StateExecs caxn't be created.", response.contains("can not be created"));
    }
     ...
     * @throws InvalidIdException the invalid id exceptionCa
     */
    @Test
    public void testUpdateNormal() throws UpdateException, InvalidIdException {
        when(service.update(mockedStateExec)).thenReturn(mockedStateExec);
        when(service.getById(any())).thenReturn(mockedStateExec);
        when(jsonUtil.marshal(any())).thenReturn("");
        resource.update(mockedStateExec);
        verify(service, times(1)).update(mockedStateExec);
    }
    ...
    public void testUpdateFail() throws UpdateException, InvalidIdException {
        when(service.update(mockedStateExec)).thenThrow(UpdateException.class);
        when(service.getById(any())).thenReturn(mockedStateExec);
        when(jsonUtil.marshal(any())).thenReturn("");
        String response = resource.update(mockedStateExec);
        verify(service, times(1)).update(mockedStateExec);
    }
