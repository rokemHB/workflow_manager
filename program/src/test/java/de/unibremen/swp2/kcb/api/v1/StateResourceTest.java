...
public class StateResourceTest {
    ...
    public void testCreateNormal() throws CreationException, EntityAlreadyExistingException {
        when(service.create(any())).thenReturn(mockedState);
        when(jsonUtil.marshal(any())).thenReturn("");
        resource.create(mockedState);
        verify(service, times(1)).create(mockedState);
    }
    ...
    public void testUpdateNormal() throws UpdateException, InvalidIdException {
        when(service.update(mockedState)).thenReturn(mockedState);
        when(service.getById(any())).thenReturn(mockedState);
        when(jsonUtil.marshal(any())).thenReturn("");
        resource.update(mockedState);
        verify(service, times(1)).update(mockedState);
    }
    ...
    public void testUpdateFail() throws UpdateException, InvalidIdException {
        when(service.update(mockedState)).thenThrow(UpdateException.class);
        when(service.getById(any())).thenReturn(mockedState);
        when(jsonUtil.marshal(any())).thenReturn("");
        String response = resource.update(mockedState);
        verify(service, times(1)).update(mockedState);
    }
