...
/**
 * The type Procedure resource test.
 *
 * @author Marius
 */
public class ProcedureResourceTest {
    ...
    public void testCreateNormal() throws CreationException {
        when(service.create(any())).thenReturn(mockedProcedure);
        when(jsonUtil.marshal(any())).thenReturn("");
        resource.create(mockedProcedure);
        verify(service, times(1)).create(mockedProcedure);
    }
    ...
    public void testUpdateNormal() throws UpdateException, InvalidIdException {
        when(service.update(mockedProcedure)).thenReturn(mockedProcedure);
        when(service.getById(any())).thenReturn(mockedProcedure);
        when(jsonUtil.marshal(any())).thenReturn("");
        resource.update(mockedProcedure);
        verify(service, times(1)).update(mockedProcedure);
    }
    ...
    public void testUpdateFail() throws UpdateException, InvalidIdException {
        when(service.update(mockedProcedure)).thenThrow(UpdateException.class);
        when(service.getById(any())).thenReturn(mockedProcedure);
        when(jsonUtil.marshal(any())).thenReturn("");
        String response = resource.update(mockedProcedure);
        verify(service, times(1)).update(mockedProcedure);
    }
