...
/**
 * The type Priority resource test.
 *
 * @author Marius
 */
public class PriorityResourceTest {
     ...
     */
    @InjectMocks
    private PriorityResource resource;
     ...
     */
    @Mock
    private Priority mockedPriority;
    ...
    public void testCreateNormal() throws CreationException, EntityAlreadyExistingException {
        when(service.create(any())).thenReturn(mockedPriority);
        when(jsonUtil.marshal(any())).thenReturn("");
        resource.create(mockedPriority);
        verify(service, times(1)).create(mockedPriority);
    }
    ...
    public void testUpdateNormal() throws UpdateException, InvalidIdException {
        when(service.update(mockedPriority)).thenReturn(mockedPriority);
        when(service.getById(any())).thenReturn(mockedPriority);
        when(jsonUtil.marshal(any())).thenReturn("");
        resource.update(mockedPriority);
        verify(service, times(1)).update(mockedPriority);
    }
    ...
    public void testUpdateFail() throws UpdateException, InvalidIdException {
        when(service.update(mockedPriority)).thenThrow(UpdateException.class);
        when(service.getById(any())).thenReturn(mockedPriority);
        when(jsonUtil.marshal(any())).thenReturn("");
        String response = resource.update(mockedPriority);
        verify(service, times(1)).update(mockedPriority);
    }
