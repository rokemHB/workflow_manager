...
/**
 * The type Cardinal value resource.
 *
 * @author Marius
 */
public class CardinalValueResourceTest {

    /**
     * Injected instance of resource
     */
    @InjectMocks
    private CardinalValueResource resource;
     ...
     */
    @Mock
    private CardinalValue mockedCardinalValue;
    ...
    public void testCreateNormal() throws CreationException {
        when(service.create(any())).thenReturn(mockedCardinalValue);
        when(jsonUtil.marshal(any())).thenReturn("");
        resource.create(mockedCardinalValue);
        verify(service, times(1)).create(mockedCardinalValue);
    }
    ...
    public void testUpdateNormal() throws UpdateException, InvalidIdException {
        when(service.update(mockedCardinalValue)).thenReturn(mockedCardinalValue);
        when(service.getById(any())).thenReturn(mockedCardinalValue);
        when(jsonUtil.marshal(any())).thenReturn("");
        resource.update(mockedCardinalValue);
        verify(service, times(1)).update(mockedCardinalValue);
    }
    ...
    public void testUpdateFail() throws UpdateException, InvalidIdException {
        when(service.update(mockedCardinalValue)).thenThrow(UpdateException.class);
        when(service.getById(any())).thenReturn(mockedCardinalValue);
        when(jsonUtil.marshal(any())).thenReturn("");
        String response = resource.update(mockedCardinalValue);
        verify(service, times(1)).update(mockedCardinalValue);
    }
