...
/**
 * The type Carrier resource test.
 *
 * @author Marius
 */
public class CarrierResourceTest {
    ...
    public void testCreateNormal() throws CreationException, EntityAlreadyExistingException {
        when(service.create(any())).thenReturn(mockedCarrier);
        when(jsonUtil.marshal(any())).thenReturn("");
        resource.create(mockedCarrier);
        verify(service, times(1)).create(mockedCarrier);
    }
    ...
    public void testUpdateNormal() throws UpdateException, InvalidIdException {
        when(service.update(mockedCarrier)).thenReturn(mockedCarrier);
        when(service.getById(any())).thenReturn(mockedCarrier);
        when(jsonUtil.marshal(any())).thenReturn("");
        resource.update(mockedCarrier);
        verify(service, times(1)).update(mockedCarrier);
    }
    ...
    public void testUpdateFail() throws UpdateException, InvalidIdException {
        when(service.update(mockedCarrier)).thenThrow(UpdateException.class);
        when(service.getById(any())).thenReturn(mockedCarrier);
        when(jsonUtil.marshal(any())).thenReturn("");
        String response = resource.update(mockedCarrier);
        verify(service, times(1)).update(mockedCarrier);
    }
    ...
    public void testDeleteInvalidId() {
        ...
        String response = resource.delete(body);
        assertTrue("Respone should contain invalid id.", response.contains("Invalid request"));
    }
