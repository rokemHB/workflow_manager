...
public class StockResourceTest {
    ...
    public void testCreateNormal() throws CreationException {
        when(service.create(any())).thenReturn(mockedStock);
        when(jsonUtil.marshal(any())).thenReturn("");
        resource.create(mockedStock);
        verify(service, times(1)).create(mockedStock);
    }
    ...
    public void testUpdateNormal() throws UpdateException, InvalidIdException {
        when(service.update(mockedStock)).thenReturn(mockedStock);
        when(service.getById(any())).thenReturn(mockedStock);
        when(jsonUtil.marshal(any())).thenReturn("");
        resource.update(mockedStock);
        verify(service, times(1)).update(mockedStock);
    }
    ...
    public void testUpdateFail() throws UpdateException, InvalidIdException {
        when(service.update(mockedStock)).thenThrow(UpdateException.class);
        when(service.getById(any())).thenReturn(mockedStock);
        when(jsonUtil.marshal(any())).thenReturn("");
        String response = resource.update(mockedStock);
        verify(service, times(1)).update(mockedStock);
    }
